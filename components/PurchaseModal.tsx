    ...
    return (
        <div className="fixed inset-0 bg-slate-900/60 flex items-center justify-center z-[100] backdrop-blur-sm p-4">
            <div className="bg-white rounded-[2rem] w-full max-w-4xl shadow-2xl animate-in zoom-in duration-200 overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-emerald-600/5">
                ...
                <div className="p-6 md:p-10 overflow-y-auto bg-slate-50">
                    <div className="grid md:grid-cols-3 gap-6">
                        {CREDIT_PACKS.map((p) => (
                            <div key={p.id} className={`bg-white rounded-3xl p-6 border-2 relative flex flex-col ${p.active ? 'border-amber-500 shadow-xl shadow-amber-500/20 scale-[1.02]' : 'border-slate-200 shadow-sm hover:border-emerald-300'} transition-all`}>
                                {p.active && <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-amber-500 text-white text-[9px] font-black px-3 py-1 rounded-full uppercase tracking-widest shadow-lg whitespace-nowrap">Best Value</div>}
                                ...
                                <div className="text-center mb-6 pt-2">
                                    ...
                                    <div className="text-xs font-black text-emerald-600 uppercase tracking-wide">Credits</div>
                                </div>
                                ...
                                <button
                                    ...
                                    className={`w-full py-4 rounded-xl font-black uppercase tracking-widest text-xs flex items-center justify-center gap-2 transition-all ${p.active ? 'bg-emerald-600 text-white hover:bg-emerald-700 shadow-lg hover:shadow-emerald-500/40' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}
                                >
                                    Buy for {p.amount}
                                </button>
                            </div>
